...
Este documento resume todas as alterações feitas ao serviço de tradução automática Dita, incluindo refatoração de arquitetura, novas características, melhorias de observação e melhorias de localização.
...
- **BackendTranslationService** — Pipeline orquestrator (validação de servidor, delegação de palco, manipulação de erros)
...
- Exibe todos os sinais R eventos à medida que ocorrem
- Tipos de mensagem codificados por cores (azul=iniciado, verde=completado, vermelho=error)
...
2. ** Stage retry** (TraduçãoRetryService): 3 tentativas adicionais com 30s atrasos
3. **Retentação do bloco** (DocumentosTranslationService): Falha nos blocos de marcação para baixo
...
O Sinal O hub R é mapeado para conexões de clientes.
...
- Nova cobertura de teste adicionada para:
  ...
  - InfraestruturaTradução Orquestração de serviços
  - indexadores de placeholder jsonstringlocalizer

### Limitações Conhecidas

- o teste é ignorado ao correr em paralelo porque várias instâncias de teste partilham o mesmo ficheiro. Passa quando corre em isolamento.
...
- — Atualizado para novo parâmetro
...
- — Documentação atualizada do gasoduto
...
- Sinal As mensagens R usam o mesmo formato
...
- ** Aumento rápido execução**: Apenas os blocos Markdown alterados/falhados são retraduzidos
