...
На відміну від позиційних власників місць (, ), іменованих власників місць **language-agnostic** — перекладачі можуть переадресувати їх, щоб відповідати цільовій граматиці без розриву коду.

## Зберігання

Завірені особи мають два джерела значень:

### 1,1 км Цінності пробігу (відновлено для динамічних даних)
...
### 2,2 км Збережені значення (для напівстатичного налаштування)
...
1. **Переклад**: Власники розміщуються з безпечного токени () для запобігання їх модифікації.
...
3. **Документ очікуваних типів**: Коментарі до файлу JSON допомагають перекладачам зрозуміти контекст
...
При збереженні вкладів ЛібреТранслата автоматично керує збереженням коштів. Немає необхідності додаткового налаштування.

Для того, щоб всі переклади словників JSON прозоро підтримали іменованих власників місць.
