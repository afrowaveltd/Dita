...
A refração abordou várias preocupações com o projeto monolítico original:
...
- ** Resiliência**: Múltiplos níveis de repetição manuseiam falhas transitórias sem bloquear todo o gasoduto.
- **Observabilidade**: Cada operação significativa é relatada via SignalR para monitoramento em tempo real.
...
- Remover as chaves apagadas de cada idioma alvo
- Salvar instantâneo para próxima comparação
...
- ** JSON**: Armazenado em um arquivo ao lado do dicionário padrão (o nome varia pelo provedor de armazenamento)
- **Purpose**: Activa a sincronização incremental rastreando o que estava presente na execução anterior
...
- **Purpose**: Permite a re-tradução parcial de apenas blocos com falha
...
- As mensagens dentro de uma única execução são sequenciadas monotonicamente
...
- Usar pastas temporárias para o ficheiro I/ O
...
- Verificar mensagens SignalR são entregues a clientes conectados
...
- Confirmar espaços são preservados entre traduções

## Considerações sobre o desempenho

- ** Memória**: Por-language saving evita manter todos os dicionários na memória
- **Disk I/O**: Arquivos de metadados adicionam pequena sobrecarga mas permitem trabalho incremental
- ** Rede**: Processamento sequencial com estrangulamento evita esmagadora LibreTranslate
