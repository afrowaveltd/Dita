...
这份文件描述了为改进可维护性,可测试性和韧性而引入的Dita自动翻译系统的模块化架构.

## 设计目标

重构解决了最初单体设计的几个问题:

- ** 消除关切**: 每个翻译域(countries,JSON词典,Markdown)被隔离.
- ** 递增持久性**: 文件在翻译后立即被每个语言保存,减少了内存使用并提供了更早的结果.
- ** 反应**:多重试级处理瞬态故障而不会阻碍整个管道.
- ** 可观察性**: 每一次重大行动都通过信号R进行实时监测.
...
**关键行为**:
- 块级颗粒性:标题,段落,列表项目分别翻译
- 每个语言块成功/失败的元数据音轨
- 失败的块会在下次运行时重审, 不重新翻译成功的块
...
- ** JSON**:存储在默认词典旁边的文件(名称因存储提供者而异)
- ** 目的**:通过跟踪上一个运行中的内容,启用递增同步
...
- ** 记忆**: 语言保存防止将所有词典保存在记忆中
- ** 磁盘一/O**: 元数据文件添加了小的间接费用, 但允许渐进工作
- **Network**:有节奏的顺序处理防止压倒性自由翻译
