...
オリジナルのモノリシックなデザインにいくつかの懸念を提起しました

- **懸念の分離**: 各翻訳ドメイン(国、JSON辞書、Markdown)は分離されています.
- **増加の持続**: ファイルは、翻訳直後に保存され、メモリ使用量を減らし、以前の結果を提供します.
- **弾性**:複数の再試行レベルは、パイプライン全体をブロックすることなく、過渡障害を処理します.
- **Observability**:実時間監視のためのSignalRによってすべての重要な操作は報告されます.
...
**主な行動**:
- デフォルト言語が英語である場合: 国名はas-isとして保存されます
- デフォルト言語が他の言語の場合: デフォルト言語に翻訳された英語名
...
- ネットワークのタイムアウト、5xxエラー、および過渡障害を処理します
- HTTPクライアントの設定に組み込まれる

### レベル2 — ステージ(TranslationRetryService)

- 30秒の遅延で最大3件の試み
- HTTP レベルのレトリーが排出されると、リクエスト全体をリドライブ
...
- **Purpose**:前の実行に存在するものを追跡することで、増分同期を有効にします
...
- 言語ブロックの状態(ボリアンの配列)
- 最終更新時刻
- **Purpose**: 失敗したブロックのみの部分的な再変換を有効にします
...
- 接続されたクライアントにSignalRメッセージが配信されることを確認します
- 同時実行防止テスト(浮腫)
- 翻訳後のマークダウン構造の検証
...
- **ディスクI/O**:メタデータファイルには、小さなオーバーヘッドを追加し、増分作業を有効にします
