...
**Responsibilities**:
- Read from directory
- نام های کشور را به دیکشنری محلی به طور پیش فرض تبدیل کنید
...
**Responsibilities**:
- کلیدهای اضافه شده / متحرک با مقایسه فرهنگ لغت به طور پیش فرض فعلی با عکس فوری قبلی
...
- **JSON ** در یک فایل در کنار دیکشنری پیش فرض (نام با ارائه دهنده ذخیره سازی متفاوت است)
- **Purpose **: ایجاد همگام سازی افزایشی با ردیابی آنچه در اجرای قبلی وجود داشت

### فایل های Hash

- **Markdown ** در کنار فایل منبع
...
- خط لوله کامل با نمونه ترجمه واقعی (local) اجرا می شود
- بررسی سیگنال پیام های R به مشتریان متصل تحویل داده می شوند
...
- **Network ** پردازش های اکتشافی با تروتلینگ مانع از انتقال بیش از حد می شود
- **CPU ** هش هش هش و regex به سرعت نسبت به تاخیر ترجمه
- **SignalR**: Lightweight messages, no payload compression needed for typical reports

## مهاجرت از طراحی یکپارچه

اصل شامل تمام منطق در یک کلاس است. مسیر مهاجرت:

1. منطق کشور را استخراج کنید
2. منطق استخراج JSON
3. استخراج منطق Markdown
4. خروجی سیگنال انتشار R
5. استخراج مجدد منطق
