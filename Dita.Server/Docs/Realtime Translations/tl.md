...
Ang serbisyo ay tumatakbo sa isang iskedyul at nagpapatupad ng isang limang-stage pipeline: server facturesation, county synchronisation, JSON dictionary synchronisation, Markdown file translation, at pinapanatili ang mga resulta. Ang bawat yugto ay naglalabas ng naka-ayos na real-time na mga pangyayari sa pag-unlad sa SignalR upang ang mga magkakaugnay na kliyente ay maaaring sumunod habang ang mga administrative sa trabaho.
...
### Stage 2 — Mga Komplikasyon ng Translate
...
- Pagkatapos ng default dictionary ay reaptured, ang bawat nawawalang country entry sa bawat target language dictionary ay isinalin at naligtas **immediately per language**.
...
7. Ang mga matagumpay na isinalin na files ay sertipikado para sa istrukturang parsiyal na parsiyal na may pinagmulan (katumbas na mga halaga, listahan ng mga bagay, code block block blocks, mga link, matapang/italic markers, at HTML tags) bago ito isulat sa disk.
8. Kung lahat ng target files para sa isang source ay magtatagumpay, ang bagong hash ay iniimbak sa tabi ng source. Kung ang pagsulat sa tabi ng pinagkunan ay nabigo (halimbawa sa mga read-lamang mga standingment), ang hash ay bumabagsak pabalik sa temporary directory.
...
- Ang mga konde ng naligtas na mga file ng lokale JSON, ay nakapagligtas ng mga Markdown files, nag-tipid ng mga hash files, at fallback hash write.
- Anumang pagkakamali sa pag - iimbak na natitipon sa panahon ng pagtakbo.
- Per- language statistics (isinasalin, hindi nabilang, error count).
...
Pipeline stage ang mensahe ay nauukol sa
...
- Placeholder maskarang: Pinangalanang mga tagahawak ng lugar () sa teksto ay pansamantalang pinapalitan ng mga ligtas na token () bago isalin at ibalik pagkatapos, tinitiyak ang tamang balarila sa mga puntiryang wika.
...
- Auto-re connects sa exponential backoff kung ang koneksyon ay bumaba.
