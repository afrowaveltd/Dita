...
서비스는 일정에 실행하고 5 단계 파이프라인을 실행합니다. 서버 검증, 국가 동기화, JSON 사전 동기화, Markdown 파일 번역 및 결과를 지속합니다. 각 단계는 신호에 구조화된 순간 진전 사건을 방출합니다 R 그래서 연결된 클라이언트는 작업 진행으로 따라 수행 할 수 있습니다.
...
- 기본 사전이 업데이트 된 후, 각 대상 언어 사전의 각 누락 된 국가 입력은 번역되고 저장됩니다 ** 언어 당 즉시 **.
...
- **추가된 키** - 현재 기본 항목에 제시된 항목은 스냅샷에서 부수하지만, 이미 그 키에 대한 수동 항목을 가지고 있지 않는 모든 대상 언어로 번역됩니다.
...
- **Each target language dictionary는 번역 완료 후 즉시 저장됩니다 **, 오히려 모든 언어가 완료 될 때까지 기다리는 것보다.
...
3. 이전 실행에서 저장 된 해시 (소스 파일 옆에 파일에 저장, 또는 임시 낙하 위치)는 현재 해시와 비교됩니다.
4. 각 대상 언어의 경우, 해당 파일은 구조적 무결성을 검사합니다.
5. 누락 된 모든 대상 파일, outdated hash, 실패 구조 검증, 또는 다시 번역에 대 한 비공식 블록을 포함.
6. **Each 대상 언어는 독립적으로 번역되고 저장됩니다 ** - 체코가 성공하지만 프랑스어가 실패하면 체코 파일은 여전히 디스크로 작성됩니다.
7. 성공적으로 번역된 파일들은 소스(equal heading counts, list items, code block, blockquotes, links, bold/italic markers, HTML tags)로 구조적 패성을 위해 검증됩니다.
8. 소스의 모든 대상 파일이 성공하면 새로운 해시가 소스 옆에 저장됩니다. 소스 옆에 쓰기가 실패하면 (읽기 전용 배포에서 예를 들어), hash는 임시 디렉토리로 돌아갑니다.
9. 어떤 타겟 번역이 유효하지 않은 경우, 메타 데이터는 그 블록을 번역하지 않고 다음 실행에 대한 재향 군인.

### 5 단계 - StoringResults

통합은 조립되고 출판됩니다. 다음을 포함합니다:
...
- 회사 소개 named placeholders () 텍스트는 일시적으로 안전한 토큰으로 대체됩니다 () 번역 및 복원 된 후, 대상 언어로 정확한 문법을 보장합니다.

### 언어 검증

- 대상 언어로 번역하기 전에, 언어는 번역 서버에서 지원됩니다.
...
- **Modularity**: 각 번역 우려는 유지 보수성 및 시험성에 대한 자체 서비스에 격리되어 있습니다.
...
- **State 추적 **: Per-file metadata () 및 hash 파일은 연속 실행에 정확한 증가 작업을 가능하게합니다.
