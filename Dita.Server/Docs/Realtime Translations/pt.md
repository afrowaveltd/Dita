...
- **LocalizationTranslationService** — Detecta chaves adicionadas/removidas no dicionário padrão JSON e as traduz em idiomas de destino.
- **DocumentsTranslationService** — Traduz arquivos de documentação Markdown com rastreamento por bloco e metadados.

Cada sub-serviço opera de forma independente e relata progresso via SignalR em tempo real.

## O que o serviço faz

O serviço é executado em um cronograma e executa um pipeline de cinco estágios: validação do servidor, sincronização do país, sincronização do dicionário JSON, tradução do arquivo Markdown e persistência dos resultados. Cada etapa emite eventos estruturados de progresso em tempo real sobre Signal R para que os clientes conectados possam acompanhar o trabalho.
...
- Após a atualização do dicionário padrão, cada entrada de país em falta em cada dicionário de idioma-alvo é traduzida e salva **imediatamente por idioma**.
...
- ** Teclas adicionadas** — entradas presentes no padrão atual, mas ausentes do instantâneo — são traduzidas para cada idioma alvo que ainda não tenha uma entrada manual para essa tecla.
- ** Teclas removidas** — entradas presentes no instantâneo mas ausentes do padrão atual — são apagadas de todos os dicionários de idioma alvo.
...
Todos os dicionários são sempre armazenados com chaves ordenadas alfabeticamente e JSON recuou para legibilidade humana.

### Etapa 4 — TranslateMarkdownFiles

O serviço caminha pelas raizes de documentação configuradas (por omissão: ) e processa todos os arquivos de origem recursivamente:

1. O conteúdo do arquivo fonte é lido e um hash SHA-256 é calculado.
2. Um arquivo ao lado das faixas de origem por idioma, por bloco de status de tradução, habilitando ** re-tradução incremental** de blocos apenas falhou.
...
8. Se todos os arquivos de destino para uma fonte tiverem sucesso, o novo hash é armazenado ao lado da fonte. Se a escrita ao lado da fonte falhar (por exemplo, em implantações somente leitura), o hash volta para o diretório temporário.
9. Se qualquer tradução de destino falhar na validação, os metadados marcam esses blocos como não traduzidos para que eles sejam tentados novamente na próxima execução.
...
Fase pipeline a mensagem pertence a
...
- Se uma solicitação de tradução falhar após as tentativas internas da LibreTranslate, a execução de até 3 tentativas adicionais de nível de estágio com 30 segundos de atraso.
- Mascaramento do placeholder: Os placeholders nomeados () no texto são temporariamente substituídos por tokens seguros () antes da tradução e restaurados depois, garantindo a gramática correta em línguas-alvo.
...
- As línguas não suportadas são ignoradas com um aviso, evitando tentativas repetidas e falhadas.
...
- Exibe o estado da conexão, a contagem de mensagens e uma tabela ao vivo de todos os eventos.
...
- ** Resiliência**: Vários níveis de repetição (HTTP, estágio, bloco) garantem que as falhas transitórias não bloqueiam o gasoduto.
- ** Rastreamento do estado**: Os metadados por arquivo () e arquivos hash permitem um trabalho incremental preciso em sequências.
