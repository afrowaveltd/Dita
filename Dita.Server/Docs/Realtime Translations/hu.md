...
Minden egyes alszolgáltatás függetlenül működik, és valós időben jelenti a SignalR-en keresztül történő előrehaladást.

## Mit tesz a szolgálat

A szolgáltatás menetrend szerint fut, és egy ötlépcsős csővezetéket hajt végre: szerver validálás, country szinkronizáció, JSON szótár szinkronizálás, Markdown fájlfordítás és az eredmények fenntartása. Minden szakasz strukturált valós idejű haladási eseményeket bocsát ki a SignalR felett, hogy a kapcsolódó ügyfelek a munka előrehaladtával folytathassák tevékenységüket.

## A csővezeték szakaszai

### 1. szakasz - Ellenőrzési szerverek

Mielőtt bármilyen fordítás munka elkezdődik, a szolgáltatás ellenőrzi, hogy minden előfeltétel teljesül:

- A konfigurációs szakasznak jelen kell lennie és érvényesnek.
- A LibreTranslate szervernek elfogadható késleltetés mellett kell reagálnia.
...
- Hiányzó lokál JSON fájlok bármilyen támogatott nyelven automatikusan létrejön.

Ha az ellenőrzés nem sikerül, a csővezeték azonnal leáll, és üzenetet bocsát ki.

### 2. szakasz - Transzlatiországok

Ország nevek tartják szinkronban egy csak olvasható katalógus () a lokalizáció JSON szótárak.

- Ha az alkalmazás alapértelmezett nyelve angol, minden ország nevét tárolja fordítás nélkül.
...
- A kézi fordítás mindig elsőbbséget élvez. Ha egy célszótár már tartalmaz egy kulcsot, akkor ez a bejegyzés változatlan marad, függetlenül attól, hogy mit mond a forrás.
...
Minden szótár mindig az ábécésorrendben válogatott billentyűkkel és az emberi olvashatóság érdekében beágyazott JSON-nal van ellátva.
...
5. Minden hiányzó célfájl, egy elavult hash, meghibásodik a szerkezet validálása, vagy tartalmaz lefordított blokkokat sorban áll a fordításhoz.
...
8. Ha egy forrás minden célfájlja sikeres, az új hash a forrás mellett kerül tárolásra. Ha az írás a forrás mellett nem sikerül (például csak olvasásra), a hash visszaesik az ideiglenes könyvtárba.
9. Ha a cél fordítás nem felel meg a validálásnak, a metaadatok ezeket a blokkokat lefordítatlannak jelölik, így a következő körben újra tesztelik őket.
...
- Számok mentett locale JSON fájlokat, mentett Marklown fájlokat, mentett hash fájlokat, és összeomlás hash írások.
...
Ha egy szakasz sem sikerül, a többi szakasz kimarad, egy üzenet jelenik meg, és végül egy üzenet lezárja a futást.
...
- Helyettesítő elfedése: A szövegben szereplő nevet a biztonságos zsetonokkal () helyettesítik a fordítás előtt, majd azt követően helyreállítják, biztosítva a helyes nyelvtant a célnyelveken.

### Nyelvi érvényesítés

- A célnyelv lefordítása előtt a szolgáltatás ellenőrzi, hogy a fordítószerver támogatja-e a nyelvet.
- A nem támogatott nyelvek kimaradnak egy figyelmeztetéssel, megelőzve az ismételt sikertelen kísérleteket.

### Jelölési blokkszint-helyreállítás

- Jelölések fordítások végzik block-by-block (címek, bekezdések, list elemek).
- Ha egy egyes blokk nem tud lefordítani, akkor a metaadat fájlban nem fordítható, és a következő csővezetéken újra kell próbálni.
...
Minden egyes hiba a jelentésben tartalmazza a forrásazonosítót (nyelvkód, fájlútvonal vagy szakasznév), a hibakódot és a humán olvasható üzenetet.
