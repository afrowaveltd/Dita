...
活口翻译Dashboard是一个管理页面,为自动翻译管道提供实时能见度. 它连接到信号R中枢,并在发生时显示所有管道事件.
...
所有来自翻译管道的信号R事件都显示在一个实时更新的表格中:

- ** 序列号**——每次输油管运行中的Monotonic计数器
- ** 时间戳**——收到活动的当地时间
- ** 运行 ID ** – 关联性图形界面缩短
...
- ** 清除种子 ** — 删除所有显示的信件并重放计数器
- ** Export JSON** — 下载所有收到的消息作为JSON文件进行分析
- ** 信箱**——显示本届会议收到的活动总数
...
- 纯 HTML/JS 有靴子5型
- 使用Microsoft SignalR JavaScript客户端库(从CDN装入)
- 事件种子不需要服务器侧渲染
