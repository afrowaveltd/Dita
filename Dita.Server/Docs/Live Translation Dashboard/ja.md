...
ライブ翻訳ダッシュボードは、自動翻訳パイプラインにリアルタイムの可視性を提供する管理者ページです。 シグナルR ハブに接続し、すべてのパイプラインイベントを表示できます.
...
翻訳パイプラインからのすべてのSignalRイベントは、ライブアップテーブルに表示されます
...
- **LocalizationHub**() — すべての接続されたクライアントにメッセージを放送するSignalRハブ
...
2. ナビゲートへ
3. 翻訳実行をトリガー(スケジューラ待ち、またはAPI呼び出し)
4. リアルタイムでイベントを見る
...
- **Filtering** — ステージ、タイプ、またはIDでイベントをフィルタリング
...
4. WebSocketメッセージのブラウザネットワークタブをチェック
